...
void verifica_hcsr04(struct Node* primeiro, struct Node* segundo , struct Node* terceiro, struct Node* quarto){
  int contador = 0;
  ...
  do{
    ...
    /*
   Serial.print("GPIO Atual");
   Serial.println(atual->GPIO_NUM);
   Serial.print("delta ");
   Serial.println(atual->delta);
   */
  }while(contador < 4);
  // Serial.print("Primeiro");
  // Serial.println(primeiro->delta);
  // Serial.print("Segundo");
  // Serial.println(segundo->delta);
  // Serial.print("Terceiro");
  // Serial.println(terceiro->delta);
  // Serial.print("Quarto");
  // Serial.println(quarto->delta);
...
void loop(){
    ...
    uint8_t led_ativo = 0;
    uint32_t reading;
    tmr = xTimerCreate("tempo", pdMS_TO_TICKS(25), true, 0, interrompe_timer);//Cria o timer com 1000ms de frequencia com auto-reaload
    ...
    Serial.println("timer iniciado");
      ...
      while (1)
      {
        Serial.println("inicio do executivo");
        verifica_hcsr04(primeiro, segundo, terceiro, quarto);
    
        espera=true;

        while(espera==true);
        //printf("Inicio do segundo ts : %lu \n",  (((unsigned long) esp_timer_get_time()) - inicio)/1000);
        ...
        while(espera==true);
       // printf("Inicio do terceiro ts : %lu \n",  (((unsigned long) esp_timer_get_time()) - inicio)/1000);
        ...
        while(espera==true);
        //printf("Inicio do oitavo ts : %lu \n",  (((unsigned long) esp_timer_get_time()) - inicio)/1000);
        
        espera=true;

        while(espera==true);
        //printf("Inicio do nono ts : %lu \n",  (((unsigned long) esp_timer_get_time()) - inicio)/1000);
        ...
        while(espera==true);
        //printf("Inicio do decimo primeiro ts : %lu \n",  (((unsigned long) esp_timer_get_time()) - inicio)/1000);
